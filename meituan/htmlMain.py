...
class MakeToken():
    ...
    def join_token(self):
        ...
        # print(str_json)
        token_decode = zlib.compress(bytes(json.dumps(str_json), encoding="utf8"))
        token = str(base64.b64encode(token_decode), encoding="utf8")
        return token
...
def getShops(name = '', url = '', page = 1):
    ...
    tempUrl = 'https:' + url + '/meishi/' + 'api/poi/getPoiList?cityName=' + cityName + '&cateId=0&areaId=0&sort=&dinnerCountAttrId=&page=' + str(page) +'&userId=&uuid=' + uuid + '&platform=1&partner=126&originUrl=' + originUrl + '&riskLevel=1&optimusCode=10'
    newUrl = tempUrl
    # if page > 1:
    ...
    token = MakeToken(areaId, name, 'https:' + url + '/meishi/', page, uuid)
    ...
    try:
        ...
        if r.get('data') and r['data']['poiInfos']:
            for item in r['data']['poiInfos']:
                ...
                try:
                    cursor.execute(sql, (item['title'], item['address'], '', item['poiId'], name, page))
                    db.commit()
                except:
                    db.rollback()
        if r.get('code') == 406:
            getShops(name, url, page)
        # print(len(r['data']['poiInfos']))
